...
function emailValidation() {
    ...
    if (email.trim() === "") {
      ...
      emailError.text("Please enter a valid email address. for ex:gunda@gmail.com");
      return false;
    } else {
      emailError.text("");
      return true;
    ...
    $.ajax({
        ...
        success: function(response) {
          if (response.includes("email already Taken!")) {
            $("#statusFail").text("Email already taken! try with different email");
            $("#myform").addClass("d-none");
            $("#registrationLink").removeClass("d-none");
