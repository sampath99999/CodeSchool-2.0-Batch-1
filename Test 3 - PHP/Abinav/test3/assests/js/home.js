  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (parsedResponse.status === true) {
        window.localStorage.setItem("email", parsedResponse.data[0].email);
        window.localStorage.setItem("role_id", parsedResponse.data[0].role_id);
        window.location.replace("./dashboard");
      } else {
        $("#loginError").text(parsedResponse.message);
      }
    },
    error: function () {
      console.log("invalid");
